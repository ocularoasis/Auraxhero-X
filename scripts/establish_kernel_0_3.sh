...
for d in "${package_dirs[@]:-}"; do
  if [ -d "$d/app" ] || [ -d "$d/src" ] || [ -d "$d/lib" ] || [ -f "$d/tsconfig.json" ] || [ -f "$d/next.config.ts" ] || [ -f "$d/next.config.js" ] || [ -f "$d/vite.config.ts" ]; then
    valid_roots+=("$d")
  fi
done
...
while IFS= read -r -d '' f; do
  ...
  if [ -n "$reason" ]; then
    printf '%s\t%s\t%s\n' "$rel" "$(sha256 "$f")" "$reason" >> "$EXCLUDED_TMP"
    rm -f -- "$f"
  fi
...

{
  ...
  while IFS=$'\t' read -r p s r; do
    [ -n "$p" ] || continue
    [ "$first" -eq 1 ] || echo ','
    jq -n --arg p "$p" --arg s "$s" --arg r "$r" '{path:$p,sha256:$s,reason:$r}'
    first=0
  done < "$EXCLUDED_TMP"
  echo ']'
} | jq -c '.' > "$HIST_INDEX"
...
while IFS= read -r -d '' src; do
  ...
  if [ -e "$dst" ] && [ -f "$src" ] && [ -f "$dst" ]; then
    ...
    [ "$src_sha" = "$dst_sha" ] || fail "canonicalization collision at $rel"
  elif [ -e "$dst" ] && [ ! -f "$src" ]; then
    fail "canonicalization collision at $rel"
  fi
done < <(find "$CANON_ROOT" -type f -print0)
...
The canonical tree is established only when exactly one application root can be identified and conflicting existing
paths are byte-identical. No source is discarded merely because it is old, experimental, incomplete, or currently
unused.
EOF
...
if [ -s "$findings" ]; then
  jq -n --arg zip "$ZIP_NAME" --arg sha "$ZIP_SHA256" --argjson findings "$(jq -R -s 'split("\n") | map(select(length>0))' "$findings")" \
    '{source:{historical_zip:$zip,zip_sha256:$sha},security:{status:"blocked",high_confidence_findings:$findings}}' > "$REPORT_JSON"
